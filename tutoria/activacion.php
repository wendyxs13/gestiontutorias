...
session_start();
 ...
 /// echo '"UPDATE ges_registro_alu SET estado ="2" WHERE (idges_registro_alu = '.$con_id.' );';
  ...
  $query_exi = "SELECT idges_registro_tutor, nombre, ap, am, num_eco, correo, estado_tutor FROM ges_registro_tutor WHERE idges_registro_tutor= ?;";
  ...
  if($estado == "0"){
    $query_activa = "UPDATE ges_registro_tutor SET estado_tutor ='1' WHERE (idges_registro_tutor = ? );";
    $stmt_activa = $connection->prepare($query_activa);
    $stmt_activa->execute(array($con_id));
    $total_act=$stmt_activa->rowCount();

    if(($total_act > 0) || ($estado == 1) ){
      ...
      echo '<script>alert("Te informamos que su cuenta se activado correctamente. Ahora puedes iniciar sesión para generar tu informe de tutoría ingresando tu correo institucional y número económico. ¡Muchas gracias por su participación! ");</script>';
