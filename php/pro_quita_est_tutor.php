<?php
session_start();
$usuario=($_SESSION['us_tutor']);
$_SESSION["us_tutor"] = $usuario;

if (!empty($_POST)) {
	$matri   =$_POST['matri'];
  $trim_codi = $_POST['trim_inf'];
  $trimestre = base64_decode($trim_codi); 
  $trimestre = htmlspecialchars($trimestre);
	$total_status = 0;

	include 'conn.php';
  $pdo = Connection::getInstance();
  $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

  $sql = "SELECT matricula from estudiante_tutor where matricula= ?;";
  $q = $pdo->prepare($sql);
  $q->execute(array($matri));
  $total=$q->rowCount(); 

  if($total > 0){
  	$query_status = "UPDATE estudiante_tutor SET no_eco = 1, status_estudiante = 6 WHERE (matricula = ? and trimestre = ?);";
    $stmt_status = $pdo->prepare($query_status);
    $total_status =  $stmt_status->execute(array($matri, $trimestre));
  }

  echo $total_status;



/*
  if($total_status > 0){
  	echo "<br><b class='ml-4'>Los datos se actualizaron correctamente.</b>";
  }else{
    echo "UPDATE estudiante_tutor SET no_eco = 1, status_estudiante = 6 WHERE (matricula = $matri and trimestre = $trimestre);";
  	echo "<br><b class='ml-4'>Problemas al actualizar el estatus.</b><br><br><a href='baja.php'><b class='ml-4'>Regresar</b>";
  }
*/



}



?>