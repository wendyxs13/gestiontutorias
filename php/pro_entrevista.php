 <?php
  ...
  if ($valido){
   ...
   // echo "total: ".$total_exi;
    ...
    if($total_exi==0){ /// if no existe entrevista del alumno inserta
        ...
        $query_estado = "UPDATE ges_registro_alu SET estado ='3' WHERE (matri_alu = ? );";
        $stmt_estado = $connection->prepare($query_estado);
        $stmt_estado->execute(array($matricula));
        $total_act=$stmt_estado->rowCount();
  ...
  if($total_ins > 0){
   ...
   /* $response = [
      'status' => 'error',
      'message' => 'Problemas al guardar, verifica ingresar correctamente tus datos'
    ];*/
  }
