...
if(isset($_SESSION['us_tutor_ad'])){
    ...
    header("Content-Disposition: attachment;filename=".$trim."_tutores_sin_informes_".$fecha.".xls");
    ...
    $query_exi = "SELECT d.num_eco as cel_01, d.nombre as cel_02, d.ap as cel_03, d.correo as cel_04, COUNT(DISTINCT ti.tutor_id) AS cel_05, COUNT(DISTINCT tg.num_eco) AS cel_06 FROM ges_registro_tutor d LEFT JOIN ges_tutoria_individual ti ON ti.tutor_id = d.num_eco AND ti.trim_informe = '$trim' LEFT JOIN  ges_tutoria_grupal_3 tg ON tg.num_eco = d.num_eco AND tg.trim_informe = '$trim' WHERE d.num_eco != 1 AND d.num_eco != 'xochito2024' GROUP BY d.num_eco, d.nombre, d.ap, d.correo HAVING cel_05 = 0 AND cel_06 = 0;";
