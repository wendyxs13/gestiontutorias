...
$con = $base->connect();
  $sql2 = "INSERT INTO accesos (fecha, hora, usuario, ip, script, tipo,comentarios) Values 
    ('" . date("Y-m-d")  . "','" . date("H:i:s")  . "','".strtoupper($_SESSION['user'])."','','INDICE DE TITULACION APARENTE','".$_SESSION['tipo']."',
...
<script type="text/javascript">
	$(document).ready(function(){
		$("#getsemaforo").submit(function(e){
			...
			$.post("./?action=getindicetitulacion",$("#getsemaforo").serialize(), function(data){
				$("#results").html(data);
			});
